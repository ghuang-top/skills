...
description: 检查 Git 暂存区（staged）改动中新增的命名（变量、函数/方法、入参、类/类型、常量等）是否规范。优先与项目现有代码风格保持一致，无先例时按语言惯例判断。纯只读 —— 只在对话中按 🔴/🟡/🟢 分级列出问题与建议命名，绝不修改仓库。当用户要求“检查命名 / 命名规范审查 / 看看 staged 里的命名规不规范 / naming check”时使用。
tools: Read, Grep, Glob, Bash
---

# Git 暂存区命名规范检查（只读）

检查当前 **Git 暂存区（staged / index）** 改动中**新增的命名**是否规范，用简体中文按 🔴/🟡/🟢 分级输出问题与建议命名。本 skill **只审命名、只输出意见**，不审查业务逻辑，不改动任何文件。
...
只检查 **staged diff 新增行（`+` 行）中新引入的标识符**：
...
- 纯粹的代码搬动：名字未变、只是位置移动（用 `git diff --staged -M` 识别重命名文件，避免把整文件搬动当成新增）
...
若暂存区为空，直接告知用户“当前没有 staged 改动可检查”，结束。

### 第 2 步：读取暂存区 diff，圈出新增命名

```bash
git diff --staged --stat        # 先看改动规模
git diff --staged -M            # 完整 staged diff（-M 识别文件重命名，避免误判）
```

- 只看 **staged** 改动；working tree 未暂存部分不在范围内。
...
每条一行，格式：**等级 — `文件:行号` — 当前命名 + 问题 → 建议改成什么名字**。只给建议的**名字本身**（附一句理由），**不给出整段重写代码**，是否采纳、如何全局替换由用户自行操作。
...
>
> 🟢 `src/order.ts:23` — `list` 可用但偏空泛 → 可考虑 `pendingOrders`
>
> **结论**：共检查 N 个新增命名，🔴 1 / 🟡 2 / 🟢 1。修正 🔴 后建议顺手统一 🟡 两处。

- 同一名字出现在多行时，只报**定义处**一条，行号指向定义。
- 全部合规时输出：“🟢 本次 staged 改动共新增 N 个命名，均符合项目惯例，无需修改。”
